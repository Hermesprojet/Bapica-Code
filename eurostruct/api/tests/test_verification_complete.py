...
def test_le_contexte_normatif_est_gele_et_entre_dans_l_empreinte(
    ...
    assert len(corps["inputs_hash"]) == 64
    assert len(corps["ndp_snapshot_id"]) == 64
    assert len(corps["fingerprint"]) == 64
    ...
    assert ligne[4] == corps["inputs_hash"]
...
def test_l_etude_exploratoire_se_rouvre_a_l_identique(
    ...
    revu = relu.json()
    assert revu["inputs_hash"] == cree["inputs_hash"]
    assert revu["ndp_snapshot_id"] == cree["ndp_snapshot_id"]
    assert revu["fingerprint"] == cree["fingerprint"]
...
def test_les_etudes_survivent_a_un_redemarrage_de_l_api(
    ...
    relu = client_neuf.get(f"{_url(projet)}/{cree['calculation_id']}",
                           headers=_entete(jeton(ACTEUR_A)))
    assert relu.status_code == 200, relu.text
    assert relu.json()["fingerprint"] == cree["fingerprint"]
...
def test_une_autre_organisation_n_obtient_aucune_lecture(
    ...
    for secret in (cree["inputs_hash"], cree["ndp_snapshot_id"],
                   cree["fingerprint"]):
...
def test_changer_le_referentiel_change_l_empreinte(
    ...
    assert be["inputs_hash"] == fr["inputs_hash"], (
        "les nombres saisis sont identiques")
    assert be["ndp_snapshot_id"] != fr["ndp_snapshot_id"]
    assert be["fingerprint"] != fr["fingerprint"]


def test_deux_requetes_identiques_ont_un_comportement_defini(
        client, jeton, projet) -> None:
    """L'IDEMPOTENCE EST DEFINIE, PAS SUPPOSEE.
    ...
    parce qu'un ingenieur qui relance veut voir qu'il a relance. Ce qui serait
    faux, c'est que la seconde ecrase la premiere en silence.
    """
