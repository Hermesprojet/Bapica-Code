...
it on a real five-section note — bending, shear, anchorage, serviceability and
the deflection exemption — rather than trusting the design.
"""
...
def designs(params_fr, params_fr_sls):
    ...
    span_depth = check_span_depth(
        section=RectangularSection(b=Q_(300, "mm"), h=Q_(600, "mm"), d=Q_(550, "mm")),
        concrete=c, steel=s, l_eff=Q_(6000, "mm"),
        system=StructuralSystem.SIMPLY_SUPPORTED,
        A_s_required=bars_area(4, 20), A_s_provided=bars_area(4, 20),
        params=params_fr, element="P1",
    )
    return flexure, shear, anchorage, sls, span_depth


@pytest.fixture
def note(designs, params_fr):
    flexure, shear, anchorage, sls, span_depth = designs
    return CalculationNote(
        ...
        sections=(
            ...
            section_from_design(
                span_depth, title="Limitation des fleches",
                basis="EN 1992-1-1 §7.4.2",
                assumptions=(span_depth.verdict,),
            ),
        ),
