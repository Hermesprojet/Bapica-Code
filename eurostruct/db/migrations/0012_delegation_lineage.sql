...
begin
  ...
  loop
    -- LE DIAGNOSTIC NOMME LE DONNEUR, et ce n'est pas un ornement: un octroi
    -- ne se revoque QUE par celui qui l'a pose (fait mesure dans ce jalon).
    -- « CREATE est reste » n'indique aucune action; « CREATE est reste, pose
    -- par X » dit exactement quoi faire, et par qui.
    for donneurs in
      select string_agg(distinct pg_get_userbyid(a.grantor), ', ') as liste
    ...
    loop
      ecarts := ecarts || format(
        ...
        'schema public, accorde par {%s}. Un octroi ne se revoque que par son '
        'donneur: si celui-ci n''est pas le role qui applique la migration, '
        'le REVOKE emis ici n''a produit qu''un WARNING',
        r.rolname, donneurs.liste);
    end loop;
  end loop;
