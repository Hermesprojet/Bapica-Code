...
begin
  ...
  loop
    if not (donneur = any (admissibles)) then
      raise exception
        'AUTHORITY_0013_GRANTOR_NOT_ADMISSIBLE: le donneur « % » de CREATE sur '
    ...
    exception when others then
      ...
      execute format('set local role %I', appelant);
      raise exception
        'AUTHORITY_0013_SCHEMA_CREATE_REVOKE_FAILED: la revocation sous le '
  ...
  if current_user <> appelant then
    raise exception
      'AUTHORITY_0013_ROLE_NOT_RESTORED: la migration s''execute encore sous '
